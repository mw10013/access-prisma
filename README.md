...
- npx prisma studio
- pg_dump -Fc -v --inserts -f access_db.dump postgresql://postgres:postgres@localhost:5432/postgres
- pg_dump -v --schema-only -f access_schema.sql postgresql://postgres:postgres@localhost:5432/postgres
- pg_dump -v --data-only --column-inserts -f access_data.sql postgresql://postgres:postgres@localhost:5432/postgres
...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly create access-0256
  fly create access-0256-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app access-0256
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app access-0256-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'access-0256-staging' does not match app name in config file 'access-0256'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name access-0256-db
  fly postgres attach --postgres-app access-0256-db --app access-0256

  fly postgres create --name access-0256-staging-db
  fly postgres attach --postgres-app access-0256-staging-db --app access-0256-staging
  ```
